...
def get_agent_usage_history(days_back: int = 90):
    ...
    AND QUERY_TEXT NOT ILIKE '%DASH_AGENT_SLACK%'
    AND QUERY_TEXT NOT ILIKE '%AGENT_USAGE_DASHBOARD%'
    AND QUERY_TEXT NOT ILIKE '%AGENT_USAGE_STAGE%'
    AND QUERY_TEXT NOT ILIKE '%_CORTEX_SEARCH_%AGENT%'
    AND TRIM(UPPER(QUERY_TEXT)) NOT LIKE 'SHOW AGENTS%'
